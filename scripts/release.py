"""Official local build/release command for SIG Linux.
...
def build_release(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if frozen_version != version:
            raise ValidationError(
                f"sig recém-gerado contém {frozen_version}, esperado {version}"
            )
...
def tests_command() -> int:
    ...
    suite.addTests(
        unittest.defaultTestLoader.discover(
            str(root / "updater_linux"), pattern="test_*.py", top_level_dir=str(root)
        )
    )
...
def updater_v2_test_command(args: argparse.Namespace) -> int:
    root = repo_root()
    updater = (args.updater or root / "updater_linux/sig_updater.sh").resolve()
    metadata_path = root / "scripts/updater_artifact.json"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Gates executáveis do SIG Linux")
