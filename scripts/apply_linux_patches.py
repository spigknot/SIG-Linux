"""Substitui o _update_script_text (PowerShell) por versão bash Linux e adapta _launch_prepared_update."""
import re
...
# Resolve o diretorio de instalacao procurando pelos marcadores de runtime
...
if ! command -v unzip >/dev/null 2>&1; then
    log "unzip nao encontrado; tentando python -m zipfile."
fi
...
# Diretorios que serao substituidos por completo
...
# Arquivos individuais (incluindo sig e sig_updater.sh)
for f in "$STAGING"/*; do
    name="$(basename "$f")"
...
print("OK - updater adaptado para Linux (bash script) + helper Windows preservado")
