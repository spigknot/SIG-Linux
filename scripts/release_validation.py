...
def validate_version_consistency(
    ...
    if frozen_version is not None and frozen_version != app_version:
        raise ValidationError(
            ...
            f"sig={frozen_version}, APP_VERSION={app_version}"
        )
...
def validate_frozen_dependencies(package_root: Path) -> None:
    ...
    try:
        ...
        candidates.append(Path(str(getattr(_sd, "_libname", ""))))
    except Exception:  # pragma: no cover - sounddevice indisponível
        pass
...
def validate_build_info(
    ...
    if data.get("sig_sha256") != sha256_file(package_root / "sig"):
        raise ValidationError("sig foi alterado depois da geração do build-info.json")
    if runtime_manifest_path is not None:
...
def validate_current(
    ...
    frozen_version = frozen_app_version(package_root / "sig")
    ...
    return [
        ...
        "sig_updater.sh corresponde ao artefato conhecido como bom",
    ]
