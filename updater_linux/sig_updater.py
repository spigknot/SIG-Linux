...
def _resolve_install_dir(target: Path, log_path: Path) -> Path:
    """Sobe até a pasta que contém os marcadores de runtime da instalação.

    Equivalente ao Resolve-SigInstallDir do updater Windows: o app pode
    passar um subdiretório e o updater encontra a raiz real da instalação.
    """
    original = target.resolve()
    current = original
    for _ in range(6):
        if (
            (current / "ffmpeg").is_file()
            or (current / "ffplay").is_file()
        ...
        if parent == current:
            break
        current = parent
    _log(log_path, f"Destino resolvido: recebido={original}; usando={original}")
    return original
...
def execute(
    ...
    target = _resolve_install_dir(target, log_path)
